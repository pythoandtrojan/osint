...
class KawaiiNetcat:
    def __init__(self, mode='client', ip=None, port=None):
        self.mode = mode
        self.ip = ip
        self.port = port
        self.sock = None
    ...
    def start_server(self):
        try:
            ...
            print(f"{UI.color('text')}Servidor ouvindo em {self.ip}:{self.port}...")
            wrapper(lambda stdscr: Animator.loading(stdscr, 2))
            
            self.conn, self.addr = self.sock.accept()
            print(f"\n{colors.GREEN}✧ Conexão estabelecida com {self.addr[0]}:{self.addr[1]} {colors.HEART}{colors.END}\n")
            return True
        except Exception as e:
            print(f"\n{colors.RED}✧ Falha no servidor: {str(e)} {colors.CRANIO}{colors.END}")
            return False
    ...
    def send(self, data):
        try:
            target = self.conn if self.mode == 'server' else self.sock
            target.send(data.encode() + b'\n')
            return True
        except:
            return False
    ...
    def receive(self):
        try:
            target = self.conn if self.mode == 'server' else self.sock
            data = target.recv(4096).decode().strip()
            if not data:
                return None
            return data
        except:
    ...
    def execute_malware(self, cmd):
        malware_cmds = {
            ...
            ╚══════════════════════════════════════╝
            {colors.END}""",
            "keylogger": f"{colors.PINK}✧ Keylogger ativado {colors.EYE}{colors.END}",
            "screenshot": f"{colors.BLUE}✧ Capturando tela do alvo...{colors.END}",
            "persist": f"{colors.PURPLE}✧ Criando persistência no sistema {colors.CRANIO}{colors.END}"
        }
        return malware_cmds.get(cmd.lower(), None)
    ...
    def server_loop(self):
        while True:
            try:
                ...
                if cmd.lower() == "exit":
                    print(f"{colors.PINK}✧ Cliente desconectado {colors.HEART}{colors.END}")
                    break
                ...
                print(f"\n{colors.ORANGE}✧ Servidor encerrado{colors.END}")
                break
...
{colors.BOLD}Uso:{colors.END}
  Modo servidor: {sys.argv[0]} -l -p <PORTA> [-i <IP>]
  Modo cliente: {sys.argv[0]} -c -i <IP> -p <PORTA>

{colors.BOLD}Opções:{colors.END}
  -l, --listen          Modo servidor
  -c, --connect         Modo cliente
  -i, --ip <IP>         Endereço IP
  -p, --port <PORTA>    Porta para escutar/conectar
...
def main():
    ...
    try:
        ...
        if not mode or not port or (mode == 'client' and not ip):
            raise ValueError("Argumentos inválidos")

        nc = KawaiiNetcat(mode=mode, ip=ip, port=port)
        nc.run()
    except Exception as e:
